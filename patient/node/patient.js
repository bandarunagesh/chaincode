...
let Chaincode = class {
  ...
  async initPatient(stub, args, thisClass) {
   ...
   // }
    ...
    let indexName = 'SSN~MSP'
    let colorNameIndexKey = await stub.createCompositeKey(indexName, [patient.SSN, patient.MSP]);
  ...
  async readPatient(stub, args, thisClass) {
   ...
   // }
    ...
    let marbleAsbytes = await stub.getState(userid); //get the marble from chaincode state
  ...
  async delete(stub, args, thisClass) {
    ...
    let userid = args[0];
    if (!userid) {
      throw new Error('userid name must not be empty');
    }
    // to maintain the color~name index, we need to read the marble first and get its color
    let valAsbytes = await stub.getState(userid); //get the marble from chaincode state
    let jsonResp = {};
    if (!valAsbytes) {
      jsonResp.error = 'userid does not exist: ' + userid;
    ...
    await stub.deleteState(userid); //remove the marble from chaincode state

    // delete the index
    let indexName = 'SSN~MSP';
    let colorNameIndexKey = stub.createCompositeKey(indexName, [marbleJSON.SSN, marbleJSON.MSP]);
